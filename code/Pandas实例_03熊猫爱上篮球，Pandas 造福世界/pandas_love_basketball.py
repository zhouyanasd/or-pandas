...
# 转换数据框列类型的函数，来自 Unit8 数据科学家 Rudolf Höhn 先生发表在
# 博客平台 Medium 的文章 "From Pandas-wan to Pandas-master"
...
shotDF = shotDF.assign(SHOT_ZONE_AREA=lambda df: df.apply(modify_area, axis=1))
...
# 解决 matplotlib Arc 无法填充颜色的问题，代码参考自 Stack Overflow：https://
# stackoverflow.com/questions/30642391/how-to-draw-a-filled-arc-in-matplotlib
...
# 代码参考了虎扑与公众号文章，并做了修改
def shot_plot(player_name='Kobe Bryant', season='2005-06',
              color='#003370', lw=2):
    # 新建一个大小为(6,6)的绘图窗口
    ...
    # 添加备注信息
    plt.title(player_name + ' ' + season + ' SEASON')
    ...
    con1 = shotDF.PLAYER_NAME == player_name
